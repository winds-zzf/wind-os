...
typedef struct MOBJCONTAINER{
	...
	//链表信息
	list_t		assigns;		//容器中已分配对象链表
	uint_t	total_num;	//容器中总共对象个数
	list_t		frees;		//容器中空闲对象链表
	uint_t	frees_num;	//容器中空闲对象个数
	list_t		extends;		//容器中拓展结构的链表
	 ...
	 */
	struct{
		list_t basic;		//容器占用的基本内存页
		list_t extends;		//容器占用的拓展内存页
		size_t size;		//连续内存页数量
	}mems;
}MObjContainer;	//0xA8
...
typedef struct MOBJEXTAND{
	...
	MObjContainer *container; 	//拓展结构所属的容器
}MObjExtend;	//0x28
...
typedef struct MObjMount{
	list_t  	containers;		//挂载容器的链表
	uint_t	containers_num;	//容器数量
	MObjContainer *cache;		//最近一次使用的容器（缓存机制，相当于每个挂载点有个影子寄存器）	
